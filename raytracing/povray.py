...
camera = Camera( 'orthographic', 'angle 90', 'location', [500, 0, 200], 'look_at', [800,0,200])
...
def scene(p, useSoller=False, showMask=True):
    ...
    # p=t #*10-20
    box = Box([p + dis + 0.001, 1.5, 1.5], [p + dis - 0.001, -1.5, -1.5])

    cylinders_thin = []
    cylinders_thick = []
    for i in range(0, len(rings)):
        cylinders_thin.append(
            Cylinder([p + dis - 0.001, 0, 0], [p + dis + 0.001, 0, 0], rings[i])
        )
        cylinders_thick.append(
            Cylinder([p + dis - 0.002, 0, 0], [p + dis + 0.002, 0, 0], rings[i])
        )
    ...
    s = Scene(
        camera,
        [
            LightSource([p, 0, 0], "color", [1, 1, 1]),
            Plane([1, 0, 0], 800, Texture(Pigment("color", [1, 1, 1]))),
        ],
    )
    if showMask:
