...
#def ScaledSphereToCartesian(r, thetaphi) seems to be unused / mystery constant alpha
...
def PointOnUnitSphere(i, j, a):
    if (i==0 and j==0):
        ...
        return np.linalg.multi_dot([ Rx(a[2]), Ry(),
                                     ScaledSphereToCartesian(ToPolarCoordinates(i*a1 + j*a2), a)
                                     ])
...
def soller(detDis, detYminus, detZminus, detYsize, detZsize, xdis1, xdis2, oAng, alphafactor, margin, ijrange, R1, R2, format='json'):
    ...
    params = [alpha, 1.001, np.radians(-15)]
    # 3rd parameter: rotation of grid around beam axis
